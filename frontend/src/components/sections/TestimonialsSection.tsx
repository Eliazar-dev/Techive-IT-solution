...
export default function TestimonialsSection() {
  ...
  return (
    <div className="bg-tag px-6 lg:px-20 py-16 lg:py-24">
      <SectionHeader tag="Client Success" title="Trusted by Engineering Leaders" />
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-[1280px] mx-auto">
        {displayItems.map((t, i) => (
          <TiltCard key={t.id ?? i} intensity={5} className="bg-bg border border-border rounded-card shadow-depth-sm hover:shadow-depth-md hover:-translate-y-1 transition-all p-8 flex flex-col gap-5">
            <Stars />
